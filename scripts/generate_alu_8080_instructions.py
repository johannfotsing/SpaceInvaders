...
def generate_add_code(op_label, syntax, num_bytes):
    ...
    if src_register != 'M'.lower():
        ...
        + "{\n"\
        + "\tcpu->a = cpu8080_ALU_add(cpu, cpu->a, cpu->{});\n".format(src_register)\
        + "\tcpu->program_counter += {};\n".format(num_bytes)\
        + "\tbreak;\n"\
        + "}"
        ...
        "case {}:\t\t\t\t\t\t\t\t\t// {}\n".format(op_label, syntax)\
        + "{\n"\
        + "\tcpu->a = cpu8080_ALU_add(cpu, cpu->a, cpu8080_read_membyte(cpu));\n"\
        + "\tcpu->program_counter += {};\n".format(num_bytes)\
        + "\tbreak;\n"\
        + "}"
        return c_instruction
...
def generate_sub_code(op_label, syntax, num_bytes):
    ...
    if src_register != 'M'.lower():
        ...
        "case {}:\t\t\t\t\t\t\t\t\t// {}\n".format(op_label, syntax)\
        + "{\n"\
        + "\tcpu->a = cpu8080_ALU_substract(cpu, cpu->a, cpu->{});\n".format(src_register)\
        + "\tcpu->program_counter += {};\n".format(num_bytes)\
        + "\tbreak;\n"\
        + "}"
        return c_instruction
        ...
        "case {}:\t\t\t\t\t\t\t\t\t// {}\n".format(op_label, syntax)\
        + "{\n"\
        + "\tcpu->a = cpu8080_ALU_substract(cpu, cpu->a, cpu8080_read_membyte(cpu));\n"\
        + "\tcpu->program_counter += {};\n".format(num_bytes)\
        + "\tbreak;\n"\
        + "}"
        return c_instruction
...
def generate_ana_code(op_label, syntax, num_bytes):
    ...
    # Actual register
    if src_register != 'M'.lower():
        c_instruction = "case {}: cpu->a &= cpu->{}; cpu8080_update_flags(cpu, cpu->a); cpu->program_counter += {}; break;\t\t\t// {}".format(op_label, src_register, num_bytes, syntax)
        return c_instruction
    # Memory cell pointed to by HL registers pair
    else:
        c_instruction = "case {}: cpu->a &= cpu8080_read_membyte(cpu); cpu8080_update_flags(cpu, cpu->a); cpu->program_counter += {}; break;\t\t// {}".format(op_label, num_bytes, syntax)
        return c_instruction
...
def generate_ani_code(op_label, syntax, num_bytes):
    ...
    """
    return "case {}: cpu->a &= opcode[1]; cpu8080_update_flags(cpu, cpu->a); cpu->program_counter += {}; break;\t\t\t// {}".format(op_label, num_bytes, syntax)
...
def generate_cmp_code(op_label, syntax, num_bytes):
    ...
    # Actual register
    if src_register != 'M'.lower():
        c_instruction = "case {}: cpu->flags.c = cpu->a < cpu->{}; cpu->flags.z = cpu->a == cpu->{}; cpu->program_counter += {}; break;\t\t// {}".format(op_label, src_register, src_register, num_bytes, syntax)
        return c_instruction
    # Memory cell pointed to by HL registers pair
    else:
        c_instruction = "case {}:\t\t\t\t// {}\n".format(op_label, syntax)\
                        + "{\n"\
                        + "\tuint8_t m = cpu8080_read_membyte(cpu);\n"\
                        + "\tcpu->flags.c = cpu->a < m;\n"\
                        + "\tcpu->flags.z = cpu->a == m;\n"\
                        + "\tcpu->program_counter += {};\n".format(num_bytes)\
                        + "\tbreak;\n"\
...
def generate_cpi_code(op_label, syntax, num_bytes):
    ...
    """
    return "case {}: cpu->flags.c = cpu->a < opcode[1]; cpu->flags.z = cpu->a == opcode[1]; cpu->program_counter += {}; break;\t// {}".format(op_label, num_bytes, syntax)
...
def generate_dcr_code(op_label, syntax, num_bytes):
    ...
    # Actual register
    if register != 'M'.lower():
        c_instruction = "case {}: cpu->{}--; cpu8080_update_flags(cpu, cpu->{}); cpu->program_counter += {}; break;\t\t\t// {}".format(op_label, register, register, num_bytes, syntax)
        return c_instruction
    # Memory cell
    else:
        c_instruction = "case {}: cpu8080_write_HL_membyte(cpu, cpu8080_read_membyte(cpu) - 1); cpu8080_update_flags(cpu, cpu8080_read_membyte(cpu)); cpu->program_counter += {}; break;\t// {}".format(op_label, num_bytes, syntax)
        return c_instruction
...
def generate_stc_code(op_label, syntax, num_bytes):
    ...
    """
    return "case {}: cpu->flags.c = 0b1; cpu->program_counter += {}; break;\t\t\t\t// {}".format(op_label, num_bytes, syntax)


def generate_cmc_code(op_label, syntax, num_bytes):
    """
    CMC instruction complements the carry bit.
    """
    return "case {}: cpu->flags.c = ~cpu->flags.c; cpu->program_counter += {}; break;\t\t\t// {}".format(op_label, num_bytes, syntax)


def generate_cma_code(op_label, syntax, num_bytes):
    """
    CMA instruction complements the accumulator byte.
    """
    return "case {}: cpu->a = ~cpu->a; cpu->program_counter += {}; break;\t\t\t\t// {}".format(op_label, num_bytes, syntax)
...
def generate_inx_code(op_label, syntax, num_bytes):
    ...
    if register_high == 'SP'.lower():
        return "case {}: cpu->stack_pointer++; cpu->program_counter += {}; break;\t\t\t\t// {}".format(op_label, num_bytes, syntax)
    else:
        register_low = REGISTER_PAIR_MAP[register_high.upper()].lower()
        c_instruction = \
        "case {}:\t\t\t\t\t\t\t\t// {}\n".format(op_label, syntax)\
